...
class App extends React.Component {
	...
	addRecord = () => {
		...
		var body = JSON.stringify({ name: this.state.name, branch: this.state.branch });
		...
		fetch("http://localhost:8000/api/view/" + id, {
			...
			.then((result) => {
				...
				this.setState({
					...
					branch: result.response[0].branch,
				});
			})
			.catch((error) => console.log("error", error));
	};
	...
	updateRecord = () => {
		...
		var body = JSON.stringify({ id: this.state.id, name: this.state.name, branch: this.state.branch });
				...
				<Container>
					...
					<Row>
						<Form>
							...
							<FormGroup>
								<FormLabel>Enter the branch</FormLabel>
								<FormControl type="text" name="branch" value={this.state.branch} onChange={this.handleChange} placeholder="Enter the branch"></FormControl>
							</FormGroup>
